  ...
  commitTable: async (table_schema: TableSchema, knex_handle: Knex) => {
    ...
    await knex_handle.schema.createTable(table_schema.name, (table) => {
      for (let col of table_schema.columns) {
        let type_procedure: TypeProcedure =
          TypeProcedureSupport.getTypeProcedure(
            col.type.name,
            col.name,
            col.type.options
          );
            ...
            switch (constraint) {
              ...
              case Constraints.AUTO_INCREMENT:
                ...
                raw_cmds.push(async function (knex: Knex) {
                  ...
                  await knex.raw(
                    `ALTER TABLE ${table_schema.name} MODIFY COLUMN ${
                      col.name
                    } ${type_procedure.getMySQLType()} auto_increment`
                  );
                });
                break;
            }
        ...
        if (col.default) {
          ...
          else col_builder.defaultTo(col.default);
        }
